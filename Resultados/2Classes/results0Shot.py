...
with open('0Shot_Llama3_12ClassesResults.json', 'r') as f:
    results = json.load(f)
teste = 0
for result in results:
    if result['prediction'].lower().rstrip(".") != 'sim':
        if result['prediction'].lower().rstrip(".").replace("ã", "a") != 'nao':
            teste += 1
            print(result)
print(teste)
y_true = [result['class_label'] if result['class_label'] == 0 else 1 for result in results]
y_pred = [0 if result['prediction'].lower().rstrip(".") == 'sim' else 1 for result in results]
